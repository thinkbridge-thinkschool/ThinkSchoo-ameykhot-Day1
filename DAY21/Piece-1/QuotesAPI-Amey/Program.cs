...
if (!string.IsNullOrWhiteSpace(redisConnStr))
{
    builder.Services.AddStackExchangeRedisCache(o =>
    {
        ...
        o.InstanceName = "quotes:";          // key prefix in Redis — e.g. "quotes:quote:1"
    });
}

// AddHybridCache registers the HybridCache singleton on top of whatever IDistributedCache
// is available (Redis above, or the default no-op when Redis isn't configured).
// Stampede protection is built in: concurrent GetOrCreateAsync calls for the same key
// let only ONE factory (the DB call) execute; all others await and reuse its result.
#pragma warning disable EXTEXP0018  // HybridCache is still in preview API surface in some runtimes
...
// ── Background Jobs ────────────────────────────────────────────────────────
// Channel<T> is .NET's built-in in-memory queue (no NuGet needed).
// UnboundedChannel + SingleReader = one BackgroundService drains it.
//
// When to use Hangfire instead:
//   Use Hangfire when jobs must survive a process restart, need automatic retry
//   on failure, require a cron schedule, or you need a UI to inspect job history —
//   hosted services are correct for in-process fire-and-forget that can be lost on shutdown.
