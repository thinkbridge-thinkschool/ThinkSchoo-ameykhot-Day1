# QuotesAPI - ASP.NET Core 10 Minimal API + Angular 21 Frontend
...
The frontend is an **Angular 21** standalone app with lazy-loaded routing, functional auth guards, and View Transitions.
...
| Feature | Details |
|---|---|
| **Lazy-loaded routes** | `quote-detail`, `create-quote-page`, `login`, `not-found` all load as separate JS chunks |
| **Functional auth guard** | `authGuard: CanActivateFn` — checks `localStorage['auth_token']` + decodes JWT expiry |
| **Route params** | `/quotes/:id` reads `id` via `toSignal(route.paramMap.pipe(...))` — reactive, not snapshot |
| **View Transitions** | `provideRouter(routes, withViewTransitions())` — smooth fade between list and detail |
| **Session expiry** | Guard decodes JWT `exp` claim; expired token → `auth.logout()` → `/login?reason=expired` |
| **Login banners** | `?reason=expired` → red banner; `?reason=unauthenticated` → amber banner |
| **Password toggle** | Eye icon in login form to show/hide password |
| **Logged-in user display** | JWT decoded in `AuthService.currentUserEmail` — shows avatar + email in the header |

### Routes defined (`src/app/app.routes.ts`)

```typescript
{ path: '',            redirectTo: 'quotes', pathMatch: 'full' },
{ path: 'quotes',      component: QuotesListComponent,    canActivate: [authGuard] },
{ path: 'quotes/:id',  loadComponent: () => import('./quote-detail/...'), canActivate: [authGuard] },
{ path: 'create-quote',loadComponent: () => import('./create-quote-page/...'), canActivate: [authGuard] },
...
chunk-5U43CPMG.js   | quote-detail-component      | 12.99 kB  ← separate chunk
chunk-I6ATEXJJ.js   | not-found-component         |  5.67 kB
```

### Bug caught and fixed

**Task brief said:** check `localStorage['access_token']`  
**Existing `AuthService` uses:** `TOKEN_KEY = 'auth_token'`  
**Impact:** Guard would always redirect to login even after a successful login.  
**Fix:** Read `localStorage.getItem('auth_token')` — same key the login flow writes to.

### Verification screenshots

| # | What | Screenshot |
|---|---|---|
| 1 | Guard redirects unauthenticated to `/login?reason=unauthenticated` | `ScreenShots/01-guard-redirect-unauthenticated.png` |
| 2 | Guard passes after login, `/quotes/1` loads | `ScreenShots/02-guard-pass-authenticated.png` |
| 3 | Lazy chunk `quote-detail-component` visible in Network tab | `ScreenShots/03-lazy-chunk-network.png` |
| 4 | Route param `/quotes/1` loads correct quote with `#1` | `ScreenShots/04-route-param-quote-detail.png` |
| 5 | Invalid param `/quotes/abc` shows "Invalid quote ID in the URL." | `ScreenShots/05-invalid-param-error.png` |
| 6 | Bug fix — `auth_token` vs `access_token` key mismatch in guard | `ScreenShots/06-bug-fix-auth-guard.png` |
| 7a | Non-existent `/quotes/99999` shows HTTP 404 error | `ScreenShots/07a-quote-not-found-404.png` |
| 7b | `quote.model.ts` `id` field — rename breaks selectQuote navigation | `ScreenShots/07b-quote-model-id-field.png` |

### What breaks if API changes

- **`id` renamed to `quoteId`** → `selectQuote(quote.id)` returns `undefined` → navigates to `/quotes/undefined`
- **`/api/quotes/:id` endpoint removed** → all detail pages return 404 (no compile-time warning)
- **`auth_token` key changes** → guard always redirects — authenticated section unreachable

### What I learned

- Lazy loading is a **build-time guarantee** — the chunk name appears in `ng build` output before you even open a browser.
- Guards must read from the **same localStorage key** the login flow writes to — a mismatch is invisible in unit tests, only caught end-to-end.
- `toSignal(paramMap.pipe(map(...)))` is the correct reactive pattern for route params — `snapshot` only reads once and misses navigation changes.
- `withViewTransitions()` gives smooth page transitions in a single line of config.

See full submission details and all code: [SOLUTION.md](../Piece-1-Routing,%20lazy%20loading,%20guards/SOLUTION.md)
