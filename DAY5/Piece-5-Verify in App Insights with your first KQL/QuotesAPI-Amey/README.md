...
## Day 5 – Piece 4: Deploy via azd CLI

Deployed the full QuotesAPI to Azure Container Apps using a single `azd up` command — builds Docker image, pushes to Azure Container Registry, provisions all infrastructure via Bicep, and deploys in one step.
...
| File | Purpose |
|---|---|
| `azure.yaml` | Maps the .NET service to Container Apps host |
| `infra/main.bicep` | Subscription-scope Bicep; creates resource group |
| `infra/resources.bicep` | Provisions ACR, Log Analytics, Container Apps Env, Container App |
| `Dockerfile` | Multi-stage build: SDK → ASP.NET runtime on port 8080 |

### Deploy command

```bash
azd env new quotes-amey --subscription <id> --location centralindia
azd up --no-prompt
# SUCCESS: provisioned and deployed in 3 minutes 44 seconds
```

See [SOLUTION.md](../SOLUTION.md) for the full terminal output, azure.yaml content, what I learned, and what would break this.

---

## Day 5 – Piece 3: Azure Container Apps Fundamentals

Provisioned an **Azure Container Apps Environment** using the Azure CLI — the shared networking, logging, and runtime boundary for all Container Apps.
...
# Get JSON output for submission
az containerapp env show -n thinkschool-env -g thinkschool-rg > env-output.json

# Cleanup (after submission — stops billing)
az group delete -n thinkschool-rg --yes --no-wait
```

### What a Container Apps Environment is

A Container Apps Environment is the shared infrastructure layer — **not a container itself**. It provides:

- **Shared networking**: All apps inside communicate via internal service names
- **Shared logging**: One Log Analytics workspace captures logs from every app
- **Managed runtime**: Azure handles the underlying Kubernetes cluster; you never touch it

See [SOLUTION.md](SOLUTION.md) for the full submission including JSON output, what I learned, and what would break this.

---

## Day 5 – Piece 2: Container Image from `dotnet publish` (No Dockerfile)

.NET 10 ships built-in container image generation — no Dockerfile, no `FROM`, no multi-stage build.  
One command produces a runnable OCI image and loads it directly into the local Docker daemon.

### Container properties added to `QuotesApi.csproj`
...
No Dockerfile. The .NET SDK compiled, published, layered, and loaded the image into Docker automatically.

### Run the container

```bash
docker run -d -p 8080:8080 quotes-api:0.1.0
```

```
CONTAINER ID   IMAGE              STATUS          PORTS
e2e6e4589671   quotes-api:0.1.0   Up 14 seconds   0.0.0.0:8080->8080/tcp
```
...
**Real response from the running container:**

```json
{"status":"healthy","timestamp":"2026-05-23T07:02:30.3441315+00:00"}
```

See [SOLUTION.md](SOLUTION.md) for the full submission including `docker run` output, live `curl` response, and what I learned.
