...
public class AuthTokenServiceTests
{
    ...
    private static AuthTokenService BuildService(QuoteDbContext db, IClock clock, int accessLifetimeSeconds = 900)
    {
        ...
        {
            Key = "ThinkSchoolDay3UnitTestSigningKey-AtLeast32",
            AccessTokenLifetime = TimeSpan.FromSeconds(accessLifetimeSeconds)
        });

        return new AuthTokenService(db, options, clock, NullLogger<AuthTokenService>.Instance);
    }
