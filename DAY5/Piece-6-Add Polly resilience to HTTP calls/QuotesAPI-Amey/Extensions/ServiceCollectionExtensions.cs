...
public static class ServiceCollectionExtensions
{
    public static IServiceCollection AddInfrastructure(this IServiceCollection services, IConfiguration configuration)
    {
        ...
        // ── External Quotes service with Polly resilience ─────────────────────────
        var externalBaseUrl = configuration["ExternalQuotes:BaseUrl"] ?? "https://zenquotes.io";
        services
            .AddHttpClient<IExternalQuoteService, ExternalQuoteService>(
                client => client.BaseAddress = new Uri(externalBaseUrl))
            .AddResilienceHandler("default", (pipeline, context) =>
            {
                var logger = context.ServiceProvider
                    .GetRequiredService<ILogger<ExternalQuoteService>>();

                // 1. Retry: 3 attempts, exponential + jitter, log every attempt
                pipeline.AddRetry(new HttpRetryStrategyOptions
                {
                    ...
                    {
                        logger.LogWarning(
                            "HTTP retry {Attempt}/{Max} for {Url} — waiting {Delay:g} — reason: {Reason}",
                            args.AttemptNumber,
                            3,
                ...
                // 2. Circuit breaker: open after 50% failures over 30 s (min 2 requests)
                pipeline.AddCircuitBreaker(new HttpCircuitBreakerStrategyOptions
                {
                    ...
                    {
                        logger.LogError(
                            "Circuit breaker OPENED for {Duration:g} — reason: {Reason}",
                            args.BreakDuration,
                            args.Outcome.Exception?.Message
                                ?? args.Outcome.Result?.StatusCode.ToString());
                        return default;
                ...
                // 3. Total timeout: 10 s per full request (including all retries)
...
public static class EndpointExtensions
{
    public static WebApplication MapQuoteEndpoints(this WebApplication app)
    {
        ...
        quotes.MapGet("/inspire", GetInspireQuote).WithName("GetInspireQuote");
    ...
    private static async Task<IResult> GetInspireQuote(
        ...
        CancellationToken cancellationToken = default)
    {
        try
        {
            var quote = await svc.GetRandomQuoteAsync(cancellationToken);
            return quote is null
                ? Results.NotFound(new ProblemDetails { Title = "Not Found", Status = 404, Detail = "No quote available from external provider" })
                : Results.Ok(quote);
        }
        catch (Exception ex)
        {
            logger.LogError(ex, "External quote provider unavailable after all retries");
