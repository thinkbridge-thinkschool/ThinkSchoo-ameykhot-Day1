...
export class QuotesListComponent {
  ...
  constructor() {
    effect(() => {
      ...
      console.log(`[list effect] page=${page} search="${search}" trigger=${this.loadTrigger()}`);
      ...
      this.service.getQuotes(page, this.pageSize, search).subscribe({
        next: response => {
          ...
          console.log(`[list effect] ✓ loaded ${response.data.length} quotes (total ${response.pagination.total})`);
        },
        error: (err: Error) => {
          this.listError.set('Failed to load quotes: ' + (err.message ?? 'Unknown error'));
          this.isListLoading.set(false);
          console.error(`[list effect] ✗ error:`, err.message);
        },
      });
    });
  }
