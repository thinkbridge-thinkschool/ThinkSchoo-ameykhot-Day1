        ...
        try
        {
            ...
            if (!dbContext.Authors.Any())
            {
                ...
                for (int i = 0; i < authors.Count; i++)
                {
                    for (int j = 0; j < 8; j++)
                    {
                        var q = new Quote(authors[i].Name, $"{quoteTexts[i]} (#{j + 1})", DateTime.UtcNow);
...
public static class EndpointExtensions
{
    ...
    // ── Fast endpoint: single JOIN query + index on AuthorId ─────────

    private static async Task<IResult> GetFastQuotes(QuoteDbContext db)
    {
        // Fix 1 — single query with JOIN instead of N+1 loop
    ...
    // ── Slow endpoint: N+1 query pattern, no index on AuthorId ───────

    private static async Task<IResult> GetSlowQuotes(QuoteDbContext db)
    {
        // Problem 1 — loads ALL authors into memory first
        var authors = await db.Authors.ToListAsync();
        var result = new List<object>();

        foreach (var author in authors)
        {
            // Problem 2 — fires one SQL query PER author (N+1)
