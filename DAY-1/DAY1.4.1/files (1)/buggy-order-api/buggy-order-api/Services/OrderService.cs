...
namespace OrderApi.Services
{
    public class OrderService : IOrderService
    {
        ...
        private static double CalculateDiscount(double subtotal, string? promoCode, int totalOrdersPlaced)
        {
            const double highOrderThreshold = 500;
            const double mediumOrderThreshold = 200;
            const double highOrderDiscount = 0.10;
            const double mediumOrderDiscount = 0.05;
            const double loyaltyDiscount = 0.03;

            double discount = 0;

            if (subtotal > highOrderThreshold)
            {
                discount = subtotal * highOrderDiscount;
            }
            else if (subtotal > mediumOrderThreshold)
            {
                discount = subtotal * mediumOrderDiscount;
            }
            ...
            if (string.Equals(promoCode, PromoCodes.Save20, StringComparison.OrdinalIgnoreCase))
            {
                discount += subtotal * 0.20;
            }
            else if (string.Equals(promoCode, PromoCodes.Save10, StringComparison.OrdinalIgnoreCase))
            {
        ...
        private static class PromoCodes
        {
            public const string Save20 = "SAVE20";
            public const string Save10 = "SAVE10";
        }
    }
}
